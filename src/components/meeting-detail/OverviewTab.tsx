...
export function OverviewTab({ meeting, isAdmin }: Props) {
  ...
  const completionData = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!userIds.length) return { highlights: new Set<string>(), kpiEntries: new Set<string>(), commitments: new Set<string>(), slides: new Set<string>() };

      const [highlights, kpiEntries, commitments, slides] = await Promise.all([
        supabase.from("highlights").select("user_id").eq("meeting_id", meetingId).in("user_id", userIds),
        supabase.from("kpi_entries").select("user_id").eq("meeting_id", meetingId).in("user_id", userIds),
        supabase.from("commitments").select("user_id").eq("meeting_id", meetingId).in("user_id", userIds),
  ...
  const BriefStatusSection = () => {
    ...
    if (!brief) {
      return isAdmin && (meeting.status === "pre_meeting" || meeting.status === "in_progress") ? (
        <Card className="border border-border">
          <CardContent className="p-card-padding flex items-center justify-between">
            <div>
    ...
    return (
      <Card className="border border-border">
        <CardContent className="p-card-padding flex items-center justify-between flex-wrap gap-3">
